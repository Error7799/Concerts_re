...
# Adding the first band, venue, and concert (band1)
band1 = add_band(session=session, name="AC/DC", hometown="Australia", date="20/03/2021")
if band1:
    venue1 = add_venue(session=session, city="New York", title="JMA Wireless Dome")
    if venue1:
...
# Adding the second band, venue, and concert (band2)
band2 = add_band(session=session, name="The Beatles", hometown="London", date="05/10/2025")
if band2:
...
# Define second_venue here
second_venue = venue2

# Aggregate relationship tests for the second venue
if second_venue:
    print(second_venue.concert_on(date="20/09/2024", session=session))
    print(second_venue.most_frequent_band(session=session))
